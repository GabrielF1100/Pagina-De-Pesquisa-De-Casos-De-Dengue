...
function addUser(firstName, lastName, email, phone, username, password) {
    ...
    var newUser = { id: Math.random().toString(36).substr(2, 9), date: data, firstName: firstName, lastName: lastName, email: email, phone: phone, username: username, password: password };
...
function deleteUser(userId) {
    var updatedUserList = userList.filter(id => id !== userId);
...
function renderUserList() {
    ...
    userList.forEach(function (user) {
        ...
        listItem.innerHTML = `<span class="user-date">(Data: ${user.date})</span>
                            ...
                            <span class="user-username">(Nome de Usuário: ${user.username})</span></span>
